...
{#if isOpen}
    ...
    >
        ...
        <div
            ...
            class="glass-panel relative w-full max-w-sm rounded-[32px] border border-white/10 bg-[#020617]/90 p-8 shadow-2xl transition-all duration-500"
        >
            <div class="pointer-events-none absolute inset-0 rounded-[32px] bg-gradient-to-b from-white/10 to-transparent opacity-30"></div>
            ...
            <div class="relative z-10 mb-10 flex justify-center gap-4 {error ? 'animate-shake' : ''}">
                {#each Array(4) as _, i}
                    <div
                        class="h-3 w-3 rounded-full border-2 transition-all
                        {pin.length > i
                            ? 'border-emerald-400 bg-emerald-400 shadow-[0_0_12px_rgba(16,185,129,0.5)]'
            ...
            <div class="relative z-10 grid grid-cols-3 gap-4">
                {#each ['1', '2', '3', '4', '5', '6', '7', '8', '9', 'BACK', '0', 'DEL'] as key}
                    <button
                        ...
                        class="modal-text flex h-16 w-full items-center justify-center rounded-2xl border border-white/[0.05] bg-white/[0.03] text-xl transition-all hover:bg-white/[0.08] active:scale-90"
                    >
                        {#if key === 'DEL'}
