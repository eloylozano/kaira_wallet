<script lang="ts">
    ...
    let spent = $derived(
        (transactionsStore.all ?? [])
            .filter(t => t.type === 'expense' && t.is_paid && isCurrentMonth(t.date))
            .reduce((acc, t) => acc + Number(t.amount || 0), 0)
    );

    let investments = $derived(
        (transactionsStore.all ?? [])
            .filter(t => t.type === 'invest' && t.is_paid && isCurrentMonth(t.date))
            .reduce((acc, t) => acc + Number(t.amount || 0), 0)
    );
    ...
    // Color del texto del balance restante
    let textStatusColor = $derived(() => {
        if (remaining < 0) return 'text-rose-500';
        if (remaining < (budget * 0.1)) return 'text-orange-400';
        ...
        <div class="relative z-10 flex flex-col gap-3">
            ...
            <div class="flex items-end justify-between">
                <div class="flex flex-col">
                    <p class="text-[9px] font-black tracking-[0.15em] opacity-40 uppercase">
                        Presupuesto Mensual
                    </p>
            ...
            <div class="flex justify-between text-[9px] font-bold uppercase opacity-40">
                <span>{spent.toFixed(2)}€ gastados</span>
                <span>{investments.toFixed(2)}€ invertidos</span>
