...
{#if isOpen}
<div transition:fade class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
    <button class="absolute inset-0 cursor-default" onclick={onTouchOutside}></button>

    <div transition:scale={{start: 0.9, duration: 300}} 
         class="glass-panel relative w-full max-w-sm rounded-[32px] p-8 border border-white/10 shadow-2xl transition-colors duration-500
                bg-[#020617]/90 dark:bg-[#020617]/90 light:bg-white/90">
        
        <div class="text-center mb-8">
            <h3 class="font-bold text-lg modal-text">
                {step === 1 ? 'Verifica PIN Actual' : 'Nuevo PIN de Acceso'}
            </h3>
            <p class="opacity-40 text-xs mt-1 uppercase tracking-widest modal-text">
        ...
        <div class="flex justify-center gap-4 mb-10 {error ? 'animate-shake' : ''}">
            {#each Array(4) as _, i}
                <div class="h-3 w-3 rounded-full border-2 transition-all 
                    {pin.length > i 
                        ? 'bg-primary border-primary shadow-[0_0_10px_var(--primary-glow)]' 
            ...
            {#each ['1','2','3','4','5','6','7','8','9','BACK','0','DEL'] as key}
                <button 
                    onclick={() => {
                        ...
                        else if (key !== '') addNumber(key);
                    }}
                    class="h-16 w-full text-xl modal-text glass-button rounded-2xl flex items-center justify-center active:bg-primary/20 transition-all active:scale-90"
                >
                    {#if key === 'DEL'}
                        <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 4H8l-7 8 7 8h13a2 2 0 0 0 2-2V6a2 2 0 0 0-2-2z"/><line x1="18" y1="9" x2="12" y2="15"/><line x1="12" y1="9" x2="18" y2="15"/></svg>
                    {:else if key === 'BACK'}
