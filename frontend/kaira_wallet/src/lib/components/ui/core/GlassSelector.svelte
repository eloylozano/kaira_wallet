...
<div class="relative z-20 flex w-full flex-col gap-1.5" bind:this={container}>
	...
	<button
		type="button"
		onclick={toggle}
		class="relative flex h-12 w-full items-center justify-between rounded-[20px]
	...
	>
		...
		<svg
			...
			{isOpen ? 'rotate-180 text-primary' : 'opacity-40'}"
		>
			<path d="m6 9 6 6 6-6" />
		</svg>
	</button>
...
{#if isOpen}
	<!-- Backdrop bloquea clicks detrás -->
	<button
		type="button"
	...
	<div
		...
		backdrop-blur-lg"
	>
		<div class="custom-scrollbar max-h-[280px] overflow-y-auto py-1">
			{#each options as opt}
				<button
					type="button"
					...
					{value === opt.value ? 'bg-primary font-black text-black' : 'text-white/70 hover:bg-white/5'}
					"
				>
