...
<div class="kaira-panel flex items-center gap-4 rounded-3xl border border-white/5 bg-white/5 p-4">
	...
	<div class="min-w-0 flex-1">
		...
		<div class="flex items-center justify-between gap-2">
			<p class="text-[8px] font-bold tracking-wider uppercase opacity-40">
				{tx.category?.name}
			</p>
			<div class="flex items-center gap-2">
				<div class="h-1 w-1 rounded-full bg-white/10"></div>
				<p class="text-[8px] font-bold text-white/30 uppercase">
					{formattedDate()}
				</p>
			</div>
		</div>
	...
	<div class="text-right">
		...
		{#if tx.is_paid === false}
			<p class="text-[9px] font-black tracking-tighter text-orange-400/70 uppercase">Pendiente</p>
		{/if}
	</div>
</div>
