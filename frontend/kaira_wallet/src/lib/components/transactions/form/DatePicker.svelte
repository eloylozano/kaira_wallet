<script lang="ts">
	...
	$effect(() => {
		...
		loadFromValue(value);
		if (fpInstance) fpInstance.setDate(value, false);
	});
	...
	onMount(() => {
		// Detección robusta adaptada a las directivas de Safari en iOS
		isMobile =
			/Android|iPhone|iPad|iPod/i.test(navigator.userAgent) ||
			(navigator.maxTouchPoints > 0 && window.matchMedia('(max-width: 1024px)').matches);

		if (!isMobile) {
			// Flatpickr solo se instancia si es escritorio
			fpInstance = flatpickr(calendarBtn, {
				defaultDate: value,
				disableMobile: true,
				dateFormat: 'Y-m-d',
	...
	function handleCalendarClick() {
		// En PC, si por algún motivo falla el trigger de flatpickr, forzamos su apertura
...
<div class="space-y-3">
	...
	<div
		class="kaira-panel flex items-center justify-between rounded-2xl px-3 py-2 transition-all focus-within:border-primary/40 sm:px-4 sm:py-3"
	>
		...
		<div class="relative flex h-10 w-10 items-center justify-center">
			<button
				...
				class="absolute inset-0 flex h-full w-full items-center justify-center rounded-xl bg-white/5 text-lg transition-all active:scale-90 hover:bg-white/10"
			>
				📅
			</button>
			...
			<input
				...
				type="date"
				bind:value
				class="absolute inset-0 h-full w-full opacity-0 cursor-pointer {isMobile
					? 'block'
