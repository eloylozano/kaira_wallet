<script lang="ts">
	...
	import { getActivePin } from '$lib/config/api'; 
	const activePin = $derived(getActivePin());

	// Cargar los datos de la base de datos al iniciar o cambiar de PIN
	$effect(() => {
		if (activePin) {
			settingsStore.fetchSettings(activePin);
			investmentStore.fetchSettings(activePin);
		}
	});
	...
	$effect(() => {
		tempTarget = investmentStore.targetSavings;
	});

	function handleBudgetConfirm() {
		settingsStore.updateBudget(Number(tempBudget), activePin);
		isConfirmBudgetOpen = false;
	}

	function handleTargetConfirm() {
		investmentStore.updateTarget(Number(tempTarget), activePin);
						...
						{#each section.items as item, i}
							<div class="flex items-center justify-between px-4 py-3.5">
								...
								{:else if item.type === 'target'}
									<div class="flex items-center gap-2">
										<span class="text-xs font-bold text-white opacity-50">€</span>
										<input
											type="number"
											bind:value={tempTarget}
											class="kaira-input-settings focus:border-emerald-500/50"
										/>
