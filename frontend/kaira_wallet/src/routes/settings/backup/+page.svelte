<script lang="ts">
    ...
    async function loadLogs() {
        const response = await fetch(apiUrl('/backup/logs'), { headers: { 'X-Kaira-PIN': getActivePin() } });
    ...
    onMount(async () => {
        try {
            const [configResponse] = await Promise.all([
                fetch(apiUrl('/backup'), { headers: { 'X-Kaira-PIN': getActivePin() } }),
                loadLogs()
            ]);
    ...
    async function saveFrequency() {
        ...
        try {
            const response = await fetch(apiUrl('/backup'), {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'X-Kaira-PIN': getActivePin() },
    ...
    async function createBackup() {
        ...
        notice = null;
        try {
            const response = await fetch(apiUrl('/backup/run'), { method: 'POST', headers: { 'X-Kaira-PIN': getActivePin() } });
