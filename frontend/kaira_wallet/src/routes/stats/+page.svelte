...
<div class="mx-auto flex w-full max-w-xl flex-col gap-6 pt-10 pb-32 px-4">
    <header class="flex flex-col gap-4">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div class="flex min-w-0 flex-col">
                <h1 class="text-3xl font-black leading-tight sm:text-4xl italic">Stats</h1>
                <p class="text-[10px] font-bold tracking-[0.2em] uppercase opacity-30">Análisis de rendimiento</p>
            </div>

            <div class="flex w-full gap-2 sm:w-auto">
                <GlassSelector bind:value={statsService.selectedMonth} options={statsService.availableMonths} />
                <GlassSelector bind:value={statsService.selectedYear} options={statsService.availableYears} />
            </div>
        </div>
